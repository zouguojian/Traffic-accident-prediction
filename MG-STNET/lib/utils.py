...
def mask_loss(predicts,labels,region_mask,data_type="nyc"):  # 在训练阶段使用
    """
    
    Arguments:
        predicts {Tensor} -- predict，(B, T, W, H)
        labels {Tensor} -- label，(B, T, W, H)
...
def compute_loss(net,dataloader,risk_mask,
    ...
    for feature,target_time,graph_feature,label in dataloader:
        ...
        l = mask_loss(net(feature,target_time,graph_feature,grid_node_map),label,risk_mask,data_type) #l的shape：(1,)
...
def predict_and_evaluate(net,dataloader,risk_mask,
    ...
    for feature,target_time,graph_feature,label in dataloader:
        feature,target_time,graph_feature,label = feature.to(device),target_time.to(device),graph_feature.to(device),label.to(device)
        prediction_list.append(net(feature,target_time,graph_feature,grid_node_map).cpu().numpy())
    ...
    print(inverse_trans_pre.shape, inverse_trans_label.shape)
