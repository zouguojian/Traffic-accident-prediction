...
@torch.no_grad()
def compute_loss_vit(net,dataloader,risk_mask,global_step,device,
                data_type='nyc'):
    ...
    for feature,target_time,graph_feature,label in dataloader:
        ...
        l = mask_loss(net(feature[:,:,0,:,:],feature[:,:,1:,0,0].flatten(start_dim=1,)),label,risk_mask,data_type)#l的shape：(1,)
...
@torch.no_grad()
def predict_and_evaluate_vit(net,dataloader,risk_mask,global_step,scaler,device):
    """predict val/test, return metrics
    ...
    for feature,target_time,graph_feature,label in dataloader:
        feature,target_time,graph_feature,label = feature.to(device),target_time.to(device),graph_feature.to(device),label.to(device)
        prediction_list.append(net(feature[:,:,0,:,:],feature[:,:,1:,0,0].flatten(start_dim=1,)).cpu().numpy())
        label_list.append(label.cpu().numpy())
    prediction = np.concatenate(prediction_list, 0)
    label = np.concatenate(label_list, 0)
