...
class TemporalBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, dropout=0.2):
        ...
        self.conv1 = weight_norm(nn.Conv2d(n_inputs, n_outputs, kernel_size=(1, kernel_size),
                                           stride=stride, padding=(0, padding), dilation=dilation),dim=1)
...
class TemporalConvNet(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        batch_size, T, D, W, H = x.shape
        x = x.view(batch_size, T, D, W * H).permute(0, 2, 3, 1)    # [B, D, W * H, T]
        # dilated convolution
        x = self.network(x)
        return x.permute(0, 2, 3, 1)  # [B, W * H, T, D]