...
from model.MSTNet import GSNet
...
def train(model,
    ...
    for epoch in range(training_epoch):
        ...
        for train_feature, target_time, gragh_feature, train_label in train_loader:
            ...
            l = mask_loss(model(train_feature, target_time, gragh_feature, road_adj, risk_adj, poi_adj, grid_node_map),
        ...
        val_loss = validation(model, val_loader, risk_mask, road_adj, risk_adj, poi_adj, grid_node_map, global_step - 1,
                                device, data_type)
...
def validation(model,
               ...
               device,
               data_type):
    val_loss = compute_loss(model, val_loader, risk_mask, road_adj, risk_adj, poi_adj, grid_node_map, global_step,
                            device, data_type)
    return val_loss
...
def test(test_loader,
    ...
    test_rmse, test_recall, test_map, test_inverse_trans_pre, test_inverse_trans_label = \
        predict_and_evaluate(model, test_loader, risk_mask, road_adj, risk_adj, poi_adj, grid_node_map,
                             scaler, device)
    high_test_rmse, high_test_recall, high_test_map, _, _ = \
        predict_and_evaluate(model, high_test_loader, risk_mask, road_adj, risk_adj, poi_adj, grid_node_map,
                             scaler, device)
    print('                RMSE\t\tRecall\t\tMAP')
    print('Test            %.3f\t\t%.3f%%\t\t%.3f' % (test_rmse, test_recall, test_map))
    print('High test       %.3f\t\t%.3f%%\t\t%.3f' % (high_test_rmse, high_test_recall, high_test_map))
